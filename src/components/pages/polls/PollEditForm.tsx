...
export default function PollEditForm({
  ...
  const { data: semestersData } = useSWR("/api/semesters", async () => {
  ...
  const form = useForm<CreatePollFormValues>({
    ...
    defaultValues: {
      ...
      requireIdentification: initialData.requireIdentification,
  ...
  const onSubmit = async (data: CreatePollFormValues) => {
    setLoading(true);
    const response = await updatePollDetails(pollId, data).finally(() => {
      setLoading(false);
    });
    if (response.error) {
      toast.toast({
